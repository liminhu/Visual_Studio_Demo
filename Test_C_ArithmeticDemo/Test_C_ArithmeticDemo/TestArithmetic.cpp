...
int test_gen_sub_5F020_key12(){
	char *initDatak1 = "57D83DA48F62899A5E65A79AF288389A";//"A25E4CDA2CFDD96802A85546310D6D33";
	char *initDatak2 = "9B58507640013E4BABEA43283C296FEC";//"EBC95682D9DB5B9238694075F99CCCD5";
	int resultHexLen = strlen(initDatak1);
	int resultLen = resultHexLen / 2;
	char *rorOldData = "7579FC4F55945D5A8191D1D24D9E59DC000F6EAD94EFB9B9426231F22673817E";
	int rorDataLen = strlen(rorOldData)/2;
	char *strRorOldData = hexStringToString(rorOldData, rorDataLen * 2);
	char *strRorData = (char *)malloc(rorDataLen+1);
	memset(strRorData, 0, rorDataLen  + 1);
	for (int i = 0; i < rorDataLen; i++){
		strRorData[i] =(strRorOldData[i] * 0xD3 -26) & 0xFF;
		//"55A19A03F5E28D14396929FC5D20413AE643907DE2E361614CAC495C38AF39C0";
	}
	printfOutResult((unsigned char *)strRorData, rorDataLen);
	char *strK1 = hexStringToString(initDatak1, resultHexLen);
	char *strK2 = hexStringToString(initDatak2, resultHexLen);
	char *resultK1 = (char *)malloc(20);
	memset(resultK1, 0, 20);
	char *resultK2= (char *)malloc(20);
	memset(resultK2, 0, 20);
	for (int i = 0; i < resultLen; i++){
		resultK1[i] = strK1[i] ^ strRorData[i];
		resultK2[i] = strK2[i] ^ strRorData[i];
	}
	printfOutResult((unsigned char *)resultK1, resultLen);
	printfOutResult((unsigned char *)resultK2, resultLen);
	return 0;
}
...
int test_generate01vmK(){
	char *initData = "00459434";//"00490936"; //"00490824";//"00460432";
	char *search_str = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	int divisor = 0x3E;  //³ýÊý
	int initData_len = strlen(initData);
	int operator_num = 0,tem=0;
	char num[10];
	char result[10];
	memset(num, 0, 10);
	memset(result, 0, 10);
	for (int i = 0; i < initData_len; i++){
		tem = initData[i] - 48;
		operator_num = (operator_num * 10+tem);
		//printf("\n%02x\n", operator_num);
	}
	int tem_result = operator_num / divisor;
	int index = operator_num - tem_result*divisor;
	int k = 0;
	result[k] = search_str[index];
	while (tem_result != 0){
		operator_num = tem_result;
		tem_result = operator_num / divisor;
		index = operator_num - tem_result*divisor;
		k++; 
		result[k] = search_str[index];
	}
	k++;
	result[k] = search_str[0];
	printf("\n%s: %s\n\n",initData, reverse_string(result));
	return 0;
}

char * test_gen_middle_data(){
	char *init = "CEC24C4CF59568761F491CD695EF3B1195D676D6E6E6E6E6E6E6E6E619E6E6E6";
	init = hexStringToString(init, strlen(init));
	int DATA_LEN = 20;
...
int test_init_key_data(char *v7, char *v9, int  DATA_LEN){
	char *init = "x4BBU560C12P537I5P0P";
	strncpy(v7, init, strlen(init));
	for (int i = 0; i < DATA_LEN - 1; i++){
		v9[i] = v7[i] ^ 0x5C;
		v7[i] = v7[i] ^ 0x36;
	}
	//printfOutResult((unsigned char *)v7, DATA_LEN - 1);
	//printfOutResult((unsigned char *)v9, DATA_LEN - 1);
	return 0;
}

int test_generate_key(){
//	test_init_key_data();
	//char *v7_hex = "24681E1E09696A6C1F6D6E0C696F6B15690C6C0C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C5C";
	char *v7_hex = "4E02747463030006750704660305017F036606663636363636363636363636363636363636363636363636363636363636363636363636363636363636363636";
	int v7_len = strlen(v7_hex)/2;
	char *init_key_hex = "0123456789ABCDEFFEDCBA9876543210F0E1D2C3";
	int init_key_len = strlen(init_key_hex)/2;
	char *data = (char *)malloc(1024);
	memset(data, 0, 1024);
	char *v7 = hexStringToString(v7_hex, strlen(v7_hex));
	printfOutResult((unsigned char *)v7, 30);
	...
	printfOutResult((unsigned char *)data, 92/2);
	unsignedStringCopy((unsigned char *)data + 28, (unsigned char *)v7, v7_len);
	printfOutResult((unsigned char *)data, 92 );
	char *result = sub_4F1B0(data, data + 28);
	printfOutResult((unsigned char *)result, 92);
	/*
	strcat(data, reverse_string(tem));
	strncpy(data + 28, v7_hex, strlen(v7_hex));
	printfOutResult((unsigned char *)stringToHexString((unsigned char *)data,92), 92);
	
	printf("\n%s\n", data);
	printf("\n%s", stringToHexString((unsigned char *)data, 92));
	*/
	return 0;
}
...
int test_key(){
	...
	test_init_key_data(v7, v9, DATA_LEN);
