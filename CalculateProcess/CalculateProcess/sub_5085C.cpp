...
int  sub_5085C(char *a1, char *a2);
inline unsigned int __ROR4__(unsigned int value, int count) { return _rotr(value, count); }
char * sub_4F1B0(char * result, char *a2);
int  sub_50764(char* a1, unsigned int a2, char *a3);

void printfStrHex(char* str,int len)
{
	printf("\n\n");
	for (int i = 0; i < len; i++)
	{
...
int _tmain_2(int argc, _TCHAR* argv[]){
		...
		char r1[20] = { 0, };
		char r2[20] = { 0, };
		char* str = "";
		
		sub_50764(str, strlen(str), k1);
		sub_5085C(k1, r1);
		printfStrHex(r1, 20);
		...
		system("pause");//0xb8 ,0x84 ,0xc6 ,0x5f ,0x9b ,0x4d ,0xab ,0xb4 ,0xcb ,0xcf ,0xa9 ,0x77 ,0x81 ,0x6c ,0x88 ,0x42 ,0xf3 ,0x34 ,0xfd ,0x5f 
		return 0;
	}
...
int _tmain_my(int argc, _TCHAR* argv[]){
	printf("Hello World!!!");
	char a1[] = {
...
int StringHexChangeToNum(char* str, int length)
{
...
int  decimalismToHex(int num, char * result)
{
...
int stringToFourByteHexSringToInteger(char* string, int length){
	char temp[5];
	char result[10];
	memset(result, 0, 10);
	int i = 0;
	for (i = length - 1; i >= 0 ; i--){
		...
		else if (isascii(string[i] & 0xFF) )
		{
			sprintf(temp, "%0x", string[i]); //将100转为16进制表示的字符串。
		}
		else{
			memset(temp, 0, 5);
			decimalismToHex(string[i] & 0xFF, temp);
		}
		strcat_s(result, 10, temp);
	}
	return StringHexChangeToNum(result, 2 * length);
}
*/

int hexStringToString(char *src, char *dest, int length)
{
...
int  sub_5085C(char *a1, char * a2)
{
	...
	v13 = sub_4F128(v4);   //0x35434144
	//sprintf(temp, "%x", v13);
	//hexStringToString(temp, v11, 10);
	//strcpy(v12+4, v11);
	*(unsigned int *)(v12 + 4) = v13;
	v5 = sub_4F128(*(unsigned int *)(v2 + 24));
	*(unsigned int *)(v12) = v5;
	v6 = (v4 >> 3) & 0x3F;
	if (v6 > 55)
		v7 = 120 - v6;
...
//	printf("\n\n结果：\n\n");
	for (int i = 0; i < 92; i++)
	{
	//	printf("%02x,", v2[i] & 0xFF);
		v12[40 + i] = v2[i];
	}
	sub_50764(v12, 8u, v12+40);
	for (int i = 0; i < 92; i++)
	{
		v2[i] = v12[40 + i];
	}
	*(unsigned int *)v3 = sub_4F128(*(unsigned int *)v2); 
	*(unsigned int *)(v3 + 4) = sub_4F128(*(unsigned int *)(v2 + 4));
	*(unsigned int *)(v3 + 8) = sub_4F128(*(unsigned int *)(v2 + 8));
	*(unsigned int *)(v3 + 12) = sub_4F128(*(unsigned int *)(v2 + 12));
	v8 = sub_4F128(*(unsigned int *)(v2 + 16));
	v9 = v2 + 28;
	*(unsigned int *)(v3 + 16) = v8;
	*(unsigned int *)v2 = 0;
	*(unsigned int *)(v2 + 4) = 0;
	*(unsigned int *)(v2 + 8) = 0;
	*(unsigned int *)(v2 + 12) = 0;
	*(unsigned int *)(v2 + 16) = 0;
	*(unsigned int *)(v2 + 20) = 0;
...
int  sub_50764(char* a1, unsigned int a2, char *a3)
{
	...
	v7 = v6 + v4;
	v8 = v6 + v4 >= v6;
	*(unsigned int *)(v3 + 20) = v6 + v4;
	if (v6 + v4 < v6)
		v7 = *(unsigned int *)(a3 + 24);
	v9 = v4 >> 3;
	if (!v8)
		*(unsigned int *)(v3 + 24) = v7 + 1; //不执行
	v10 = v9 & 0x3F;
	v11 = 64 - v10;
	int tt = *(unsigned int *)(a3 + 24) + (v5 >> 29);
	*(unsigned int *)(v3 + 24) = *(unsigned int *)(a3 + 24) + (v5 >> 29);
	v12 = a1;
	if (v5 >= 64 - v10)
	{
		v14 = v3 + v10;
		...
		v13 = v11;
	//	printf("\n\n输入数据如：");
	//	getMD5Result_1(v3, v3+28);
	//	printfStrHex(v3, 92);
		sub_4F1B0(v3, v3 + 28);//69280bfb0164e5d7cd00ec95c01fd49f
		v18 =v12 + v11;
		while (1)
		{
			v19 = v18;
...
char * sub_4F1B0(char * result, char *a2)
{
	...
	do
	{
		v4 = ((*(unsigned int *)(a2 + v3) & 0x000000FF) << 24);
		*(unsigned int *)(v592 + v3) = v4;
		int y = *(unsigned int *)(v2 + 1) & 0x000000FF;
		v5 = v4 | ((*(unsigned int *)(v2 + 1) & 0x000000FF) << 16);
		*(unsigned int *)(v592 + v3) = v5;
		v6 = v5 | ((*(unsigned int *)(v2 + 2) & 0x000000FF) << 8);
		*(unsigned int *)(v592 + v3) = v6;
		v7 = *(unsigned int *)(v2 + 3) & 0x000000FF;
		v2 += 4;
		*(unsigned int *)(v592 + v3) = v6 | v7;
		v3 += 4;
	} while (v3 != 64);
